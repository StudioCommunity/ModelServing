...
def load_model(model_path, github = None, module_path = None, model_class = None, *args, **kwargs):
    ...
    Example:
        model = azureml.designer.modelspec.load('StudioCommunity/CustomModules:master', 'dstest/dstest/python/dummy.py', 'DummyPythonModel', pretrained=True)
    """
    if model_path:
...
def _test_dummy_model():
    ...
    model1 = load_model(model_path, module_path="azureml.designer.modelspec.python", model_class="DummyPythonModel")
...
# python -m azureml.designer.modelspec.python
