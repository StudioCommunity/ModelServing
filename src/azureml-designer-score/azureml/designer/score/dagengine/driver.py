...
def init():
    ...
    schema_file = 'studio.swagger.json'
    ...
    if os.path.exists(schema_file):
        with open(schema_file, 'r') as fp:
            swagger_string = fp.read()
        swagger_string = swagger_string.replace(
            '$SERVICE_PATH_PREFIX$', service_path_prefix)
        if swagger_string:
            swagger_json = json.loads(swagger_string)
    if not swagger_json:
        swagger_json = generate_service_swagger(
